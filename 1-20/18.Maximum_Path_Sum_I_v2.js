...
let arrayOfLargest = [], tempLarge, largestIndex = 0;

// console.log(arrayOfArr);
for (let i = 0; i < arrayOfArr.length; i++) {
        if((arrayOfArr[i][largestIndex]*tempLarge) < (arrayOfArr[i][largestIndex+1]*tempLarge)) largestIndex += 1;
        tempLarge = arrayOfArr[i][largestIndex];
    arrayOfLargest.push(tempLarge);
}

const largestSum = arrayOfLargest.reduce((acc, cur) => acc + cur);

console.log(`The numbers which make up the maximum total from top to bottom of the triangle in question are: \n${arrayOfLargest.join('+')} = ${largestSum}`);
